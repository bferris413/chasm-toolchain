...
pub(crate) fn codegen(modname: impl AsRef<str>, ast: AssemblyAst<'_>) -> Result<AssemblyModule> {
    ...
    for node in ast.nodes.into_iter() {
        match node.kind {
                    ...
                    RefKind::LocalRef(member) => {
                        match labels.get(&member.0) {
                            ...
                            None => {
                                ...
                                match local_patches.get_mut(&member.0) {
                                    Some(patches) => patches.push(patch),
                                    None => {
                                        local_patches.insert(member.0.clone(), vec![patch]);
                                    }
                                }
            ...
            NodeKind::DefinitionRef(DefinitionRef { size, kind }) => {
                match kind {
                    RefKind::LocalRef(member) => {
                        match definitions.get(&member.0) {
                            ...
                            None => {
                                ...
                                bail!("Undefined definition '{}'", member.0);
                            }
                        }
                    }
                    RefKind::ModuleRef(module_ref) => {
            ...
            match patch {
                ...
                AssemblerPatch::Branch(branch_patch) => {
                    ...
                    generate_branch(
                        ...
                        &mut branch_bytes,
                        &labels,
                        &HashMap::new(), // no module labels yet
                        &mut HashMap::new(),
                        Some(place),
                    )?;

                    code[*place..*place + branch_bytes.len()].copy_from_slice(&branch_bytes);
                    linker_patches.push(LinkerPatch::BranchWithImport(branch_patch));
                }
                AssemblerPatch::BranchWithLink(branch_patch) => {
                    ...
                    generate_branch_with_link(
                        ...
                        &mut branch_bytes,
                        &labels,
                &HashMap::new(), // no module labels yet (linker step)
                        &mut HashMap::new(),
                        Some(place),
                    )?;

                    code[*place..*place + branch_bytes.len()].copy_from_slice(&branch_bytes);
                    linker_patches.push(LinkerPatch::BranchLinkWithImport(branch_patch));
                }
...
fn generate_instruction(
    ...
    match instr {
        ...
        Instruction::Branch { reference, cond } => {
            ...
            let should_linker_patch = generate_branch(
                &reference,
                &cond,
                output,
                labels,
            ...
            if should_linker_patch {
                let patch = LinkerPatch::BranchWithImport(BranchPatch {
        ...
        Instruction::BranchWithLink { reference, cond } => {
            let not_as_patch = None;
            let branch_instr_addr = output.len();
            let should_linker_patch = generate_branch_with_link(
                &reference,
                &cond,
                output,
                labels,
            ...
            if should_linker_patch {
                let patch = LinkerPatch::BranchLinkWithImport(BranchWithLinkPatch {
                    patch_at: BaseOffset(branch_instr_addr),
                    reference,
                    cond,
                });
...
fn generate_mov(
    ...
    match hex_literal {
        HexLiteral::U32(value) => {
            ...
            generate_instruction(movw_instr.clone(), output, &HashMap::new(), &mut HashMap::new(), &mut Vec::new())?;
            generate_instruction(movt_instr.clone(), output, &HashMap::new(), &mut HashMap::new(), &mut Vec::new())
        }
        v @ HexLiteral::U16(..) => {
            let movw_instr = Instruction::Movw {
                dest: reg,
...
pub (crate) fn generate_branch_with_link(
    ...
    let target_addr = match &reference.kind {
        RefKind::ModuleRef(modref) => {
            let maybe_val = module_labels.get(&modref.module.0)
                .and_then(|labels| labels.get(&modref.member.0));
            ...
            match local_labels.get(&local_ref.0) {
                ...
                None => {
                    if patch_offset.is_some() {
                        // we're expected to be patching - if the label isn't present now then it never will be
                        bail!("Undefined reference '{local_ref}'");
                    }

                    // forward reference - store the info and use a dummy target to be patched later
                    //
                    // We will need a linker patch, but we won't generate it until we do the assembly patch
                    ...
                    match unresolved_refs.get_mut(&local_ref.0) {
                        Some(patches) => patches.push(patch),
                        None => {
                            unresolved_refs.insert(local_ref.0.clone(), vec![patch]);
                        }
                    }
    ...
    match cond {
        ...
        None => {
            ...
            let mut bot_half: u16 = 0b11_0_1_0_00000000000;
...
pub (crate) fn generate_branch(
    ...
    match cond {
        ...
        None => {
            ...
            let encoded_branch: u16 = base_instr | masked_offset;
            output.extend(&encoded_branch.to_le_bytes());
        }
    }
